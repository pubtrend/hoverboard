...
// Timer0 overflow ISR for millis/micros
#include <avr/interrupt.h>
ISR(TIMER0_OVF_vect) {
  // Timer0: 8-bit, prescaler 64, F_CPU = 16MHz
  // Overflow every 256 ticks: 16MHz/64/256 = ~976.56Hz (~1.024ms per overflow)
  timer0_millis += 1;
  timer0_micros += 1024;
}
...
// ======= PWM2 and D4 control (8-bit timer0) ===================
  ...
  OCR0A=200; // LIFT FAN (currently at ~78% power)
  OCR0B=150; // THRUST FAN (currentl at ~60% power)
